...
const ToolboxTracker = () => {
  ...
  const [coAnimator, setCoAnimator] = useState('');
    ...
    if (editingSession) {
      const updatedSessions = toolboxSessions.map(s => 
        s.id === editingSession.id 
          ? { ...s, date: currentDate, shift, supervisor, coAnimator: coAnimator || null, theme: theme.trim(), workers: selectedWorkers, attendance: selectedWorkers.length }
          : s
      );
  ...
  const editSession = (session) => {
    ...
    setCoAnimator(session.coAnimator || '');
  ...
  const downloadCSV = () => {
    ...
    csv += "SESSIONS\n";
    csv += "ID,Date,Équipe,Superviseur,Co-animateur,Thème,Nombre de participants,Travailleurs\n";
    toolboxSessions.forEach(session => {
      csv += `${session.id},${session.date},${session.shift},${session.supervisor},"${session.coAnimator || '-'}","${session.theme || 'N/A'}",${session.attendance},"${session.workers.join('; ')}"\n`;
    });
        ...
        <div className="bg-white rounded-lg shadow-2xl p-8 max-w-md w-full">
          ...
          <div className="space-y-4">
            <div>
              ...
              <select value={loginName} onChange={(e) => setLoginName(e.target.value)} className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none" style={{borderColor: loginName ? COMPANY_COLOR : '#d1d5db'}}>
            ...
            <div>
              ...
              <input type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleLogin()} placeholder="Entrer le mot de passe" className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none" style={{borderColor: loginPassword ? COMPANY_COLOR : '#d1d5db'}} />
            </div>

            {loginError && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">{loginError}</div>}

            <button onClick={handleLogin} className="w-full text-white py-3 rounded-lg transition-colors font-bold text-lg" style={{backgroundColor: COMPANY_COLOR}}>
              Se connecter
            </button>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-lg mb-6 no-print">
          <div className="flex border-b overflow-x-auto">
            {[
              ...
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-6 py-4 font-semibold transition-colors whitespace-nowrap ${activeTab === tab.id ? 'border-b-4 text-white' : 'text-gray-600 hover:bg-blue-50'}`} style={activeTab === tab.id ? {borderColor: COMPANY_COLOR, backgroundColor: COMPANY_COLOR} : {}}>
                <span>{tab.icon}</span>
                {tab.label}
              </button>
            ))}
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input type="date" value={currentDate} onChange={(e) => setCurrentDate(e.target.value)} className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none" style={{borderColor: COMPANY_COLOR}} />
                </div>

                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">👤 Superviseur Principal</label>
                  <select value={supervisor} onChange={(e) => setSupervisor(e.target.value)} className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none" style={{borderColor: COMPANY_COLOR}}>
                    {SUPERVISORS.map(sup => (
                      <option key={sup.name} value={sup.name}>{sup.name}</option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <select value={shift} onChange={(e) => setShift(e.target.value)} className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none" style={{borderColor: COMPANY_COLOR}}>
                    {SHIFTS.map(s => (
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">👥 Co-animateur (Optionnel)</label>
                  <select value={coAnimator} onChange={(e) => setCoAnimator(e.target.value)} className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none">
                    <option value="">Aucun co-animateur</option>
                    <optgroup label="─── Superviseurs HSE ───">
                      {EXTRA_ANIMATORS.map(name => (
                        <option key={name} value={name}>{name}</option>
                      ))}
                    </optgroup>
                    <optgroup label="─── Travailleurs ───">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">📝 Thème du Toolbox Talk</label>
                  <input type="text" value={theme} onChange={(e) => setTheme(e.target.value)} placeholder="Ex: Travail en hauteur, EPI, Sécurité électrique..." className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none" style={{borderColor: theme ? COMPANY_COLOR : '#d1d5db'}} />
                </div>

                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">👷 Ajouter des travailleurs</label>
                  <div className="relative">
                    <input type="text" value={workerInput} onChange={(e) => handleWorkerInputChange(e.target.value)} placeholder="Commencer à taper le nom..." className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none" style={{borderColor: workerInput ? COMPANY_COLOR : '#d1d5db'}} />
                    {suggestions.length > 0 && (
                      <div className="absolute z-10 w-full bg-white border-2 rounded-lg mt-1 max-h-48 overflow-y-auto shadow-lg" style={{borderColor: COMPANY_COLOR}}>
                        {suggestions.map(worker => (
                          <div key={worker} onClick={() => addWorker(worker)} className="px-4 py-2 hover:bg-blue-100 cursor-pointer">
                            {worker}
                          </div>
                        ))}
                      </div>
                ...
                <div className="flex gap-3">
                  <button onClick={handleSubmit} className="flex-1 text-white py-4 rounded-lg transition-colors font-bold text-lg" style={{backgroundColor: COMPANY_COLOR}}>
            ...
            <div>
              <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                ...
                <div className="flex gap-3 flex-wrap">
                  <input type="date" value={filterStartDate} onChange={(e) => setFilterStartDate(e.target.value)} placeholder="Date début" className="px-3 py-2 border-2 rounded-lg" style={{borderColor: COMPANY_COLOR}} />
                  <input type="date" value={filterEndDate} onChange={(e) => setFilterEndDate(e.target.value)} placeholder="Date fin" className="px-3 py-2 border-2 rounded-lg" style={{borderColor: COMPANY_COLOR}} />
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="text-white" style={{backgroundColor: COMPANY_COLOR}}>
                      ...
                      <th className="px-4 py-3 text-left">Co-animateur</th>
                  ...
                  <tbody>
                    {filteredSessions.map((session, idx) => (
                      <tr key={session.id} className={idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                        ...
                        <td className="px-4 py-3 border">
                          {session.coAnimator ? (
                            <span className="text-purple-700 font-medium">{session.coAnimator}</span>
                          ) : (
                            <span className="text-gray-400 text-sm">-</span>
                          )}
                        </td>
                        <td className="px-4 py-3 border">
                          <span className="font-medium text-blue-700">{session.theme || '-'}</span>
                        </td>
                        ...
                        <td className="px-4 py-3 border text-sm">
                          {session.workers.length <= 3 ? (
                            ...
                            <div>
                              <span>{session.workers.slice(0, 3).join(', ')}</span>
                              <button onClick={(e) => {
                                e.stopPropagation();
                    ...
                    return (
                      <div key={month}>
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-6">
                          <div className="h-6 rounded-full flex items-center justify-end pr-3 text-white font-bold text-sm" style={{width: `${percentage}%`, backgroundColor: COMPANY_COLOR}}>
                            {count}
                          </div>
                        </div>
                      </div>
